...
* Enter `production` as the **Name** for the environment.
* From the **Type** drop down menu, select **Kubernetes (generic)**.
* Select the **Purpose** of this environment as **Production**.
* Click **Next**.
...
* Select the production **Target environment** defined previously.
...
com.instana.plugin.cve:
  ...
  concert:
    ...
    api_key: '** Concert API key from **Lab Preparation** chapter **'
    poll_rate: 360
    ignore_cert: true
```
...
![](https://raw.githubusercontent.com/IBM/waiops-tech-jam/main/labs/concert/vulnerability/4-instana-integration/images/cve-agent.png)
...
The output from the command should contain the second to last line that says ***Activated Sensor for concertCve***. 

:::note
You can ignore previous log errors such as **cve_sensor_exception : Invalid base_url** as long as you see later 
the activation message **Activated Sensor for concertCve**
:::
